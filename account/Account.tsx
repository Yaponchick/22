...
const AccountPage: FC = () => {
    ...
    const openModal = (link: string | null) => {
        if (!link) {
            alert('Анкета ещё не опубликована');
            return;
        }
        setCurrentLink(link);
        setIsModalOpen(true);
    ...
    const handleTogglePublish = async (id: string, currentStatus: boolean): Promise<void> => {
        try {
            ...
            // Обновляем состояние анкеты локально для мгновенного отклика
                        ...
                        <table className="survey-table">
                            ...
                            <tbody>
                                {filteredSurveys.map((survey) => (
                                    <tr key={survey.id}>
                                        <td className="nameQuest" data-label="Название">
                                            <div className="title-wrapper">
                                                <div className="survey-title-name" title={survey.title}>
                                                    {survey.title}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="status-badge-container" data-label="Статус">
                                            <span className="status-badge">
                                                {!survey.isPublished ? 'Не опубликована' : 'Опубликована'}
                                            </span>
                                        </td>
                                        <td data-label="Дата создания">{formatDate(survey.createdAt)}</td>
                                        <td data-label="Избранное">
                                            <Star />
                                        </td>
